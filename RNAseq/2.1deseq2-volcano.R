...
sig_df$color <- ifelse(sig_df$pvalue < 0.05 & abs(sig_df$log2FoldChange) >=1,ifelse(sig_df$log2FoldChange > 1 ,'red','blue'),'gray')
##
library(ggplot2)
rm(p)
p <- ggplot(sig_df, aes(x = log2FoldChange, y = -log10(pvalue),col = color)) +
  geom_point() +
...
ggsave(file=paste(path2,"/",groups,"_volcano_plot.png",sep=""))
